...
export default function Editpage({
  uzerData: { photoUrl, nome, servicosPrestados, bannerImage, portfolio, _id, avaliacao },
  pedidos,
}: {
  uzerData: UzerInterface
  pedidos: Pedido[]
}) {
  ...
  async function saveData() {
    ...
    if (imageFile !== null) {
      ...
      await api
        .post(`/uzers/profilephoto`, formData, {
          headers: {
    ...
    <>
      ...
      <section className="w-full lg:mt-24 mx-auto h-full flex flex-col items-center justify-center animate-transitionY">
        <div className="bg-cinzero w-full lg:w-10/12 relative">
          <div className="w-full h-64 flex flex-col items-center justify-center gap-1 bg-cinzero rounded-xl bg-center bg-cover bg-no-repeat transition relative">
            <Image
              fill
              src={bannerImage}
          ...
          <div
            title="Mudar foto de perfil"
            onClick={changePhoto}
            className="w-32 h-32 rounded-full bg-cinzero absolute -bottom-10 left-5 shadow-lg group flex items-center justify-center transition-colors cursor-pointer"
          >
            <Image
              fill
              src={photoUrl}
              className="rounded-full object-cover group-hover:opacity-30 transition-colors bg-cinzero"
        ...
        <div className="w-10/12 flex items-center mt-24 mb-8 justify-between desktop:flex-col mobile:flex-col mdscreen:flex-col">
          <div className="flex-1 flex flex-col items-start self-start pl-2">
            <h1 className="text-2xl font-bold mb-4 flex items-center gap-2" title="Mudar nome" onClick={changeName}>
              <span>{nome}</span> <Pencil size={20} className="text-azulao cursor-pointer" />
            </h1>
            <h2 className="font-normal text-lg">
              <strong>{servicosPrestados[0].nomeServico}</strong>{" "}
            </h2>
            <h2 className="font-normal text-base">
              <strong>
                {servicosPrestados[0].tipoServico === "ambos"
                  ? "Online e Presencial"
                  : servicosPrestados[0].tipoServico.toUpperCase()}
              </strong>{" "}
            </h2>
            {servicosPrestados[0].tipoServico === "presencial" ||
              (servicosPrestados[0].tipoServico === "ambos" && (
                <h2 className="font-medium text-base">
                  Área de atuação: <strong>{servicosPrestados[0].areaAtuacao}km</strong>{" "}
                </h2>
              ))}
      ...
      <section className="w-11/12 mx-auto h-auto flex flex-col md:flex-row items-start justify-center animate-transitionY p-10 gap-4">
        <div className="md:w-1/2 bg-white p-6 rounded-3xl w-full">
          ...
          <Link
            href={`/uzers/${_id}/portfolio`}
            className="text-xl font-bold my-8 flex justify-center items-center hover:underline"
          >
            Ver Mais
          </Link>
        </div>
        <div className="md:w-1/2 w-full bg-white shadow-2xl rounded-3xl p-4 flex flex-col items-center">
          ...
          {pedidos.map((pedido, index) => (
            <CardPedido
              ...
              _id_cliente={pedido._id_cliente}
