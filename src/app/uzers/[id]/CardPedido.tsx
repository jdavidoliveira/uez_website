...
import { Camera, Check, Clipboard, Cross, Eye, FileUp, Loader, Loader2, MessageCircle, X } from 'lucide-react'
    ...
    function FinishPedido({ titulo, nome_cliente, valor, status, descricao }: FinishPedidoProps) {
        ...
        return (
            <div className="fixed z-50 w-full h-full top-0 left-0 flex items-center justify-center bg-black bg-opacity-50 animate__animated animate__fadeIn">
                <button>
                    <X className='absolute top-6 right-8' size={44} color='white' onClick={() => setShowModal(false)} />
                </button>
                <div className="bg-white w-9/12 h-5/6 rounded-[30px] p-8 py-6 gap-4 flex flex-col items-center animate__animated animate__fadeInUp animate__faster">
                    ...
                    <div className="w-11/12 flex items-center justify-center gap-3">
                        <div className="w-1/2 flex flex-col items-start gap-3">
                            ...
                            <div className='w-full'>
                                ...
                                <div className="w-full flex items-center justify-center">
                                    <button onClick={handleCopy} className="bg-white mx-auto mt-4 transition hover:scale-105 text-azulao border font-bold py-2 px-4 rounded flex items-center justify-center gap-2">
                                        <Clipboard size={24} />
                                        {clipboard.copied ? "Copiado!" : "Copiar descrição"}
                                    </button>
                                    <button onClick={sendMessage} className="bg-roxazul mx-auto mt-4 transition-colors hover:bg-azulao text-white font-bold py-2 px-4 rounded flex items-center justify-center gap-2">
                        ...
                        <div className="w-1/2 flex flex-col items-center gap-3">
                            <h1 className="text-black text-center text-xl font-bold">Coloque uma imagem do serviço (Opcional):</h1>
