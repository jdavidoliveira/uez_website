...
export default function CardPedido({
  ...
  async function fetchClientes() {
    _id_cliente !== null
      ? await api
          .get<ClienteInterface>(`/clientes/${_id_cliente}`)
  ...
  return (
    <>
      <div
        ...
        className="bg-white shadow-md rounded-xl p-6 w-10/12 m-2 transition hover:scale-105 relative group cursor-pointer"
      >
        <h3 className="text-xl font-bold mb-2">{titulo.length > 15 ? `${titulo.slice(0, 15)}...` : titulo}</h3>
        <p className="text-gray-600 mb-4">
          {clienteData ? clienteData.nome.charAt(0).toUpperCase() + clienteData.nome.slice(1) : "Não atribuido"}
        </p>
        <div
  ...
  function FinishPedido({ titulo, nome_cliente, valor, status, descricao }: FinishPedidoProps) {
      ...
      api
        ...
        .then(async () => {
          ...
          await sendNotification(
            "servFim",
            `O Uzer finalizou seu serviço, avalie-o para que ele possa receber!`,
            _id_cliente || ""
          )
    ...
    return (
      <div className="fixed z-40 w-full h-full top-0 left-0 flex items-center justify-center bg-black bg-opacity-50 animate__animated animate__fadeIn">
        <button
          title="Fechar"
          className="absolute z-50 flex items-center justify-center bg-black md:bg-transparent rounded-xl p-1 md:top-6 md:right-8 top-6 right-8"
        >
        ...
        <div className="bg-white md:w-9/12 w-11/12 rounded-[30px] px-8 py-6 gap-4 flex flex-col items-center animate__animated animate__fadeInUp animate__faster">
          ...
          <div className="w-11/12 flex lg:flex-row flex-col overflow-y-auto items-center justify-center gap-3">
            <div className="lg:w-1/2 flex flex-col items-start gap-3">
              ...
              <div className="w-full">
                ...
                <textarea
                  value={descricao}
                  readOnly
                  className="w-full bg-[#EDEDED] opacity-50 p-4 text-lg font-medium rounded-2xl md:h-auto h-28 animate__animated animate__shakeX animate__faster"
                  cols={30}
                  rows={10}
                ></textarea>
                <div className="w-full flex items-center justify-center">
                  <button
                    onClick={handleCopy}
                    className="bg-white mx-auto mt-4 transition hover:scale-105 text-azulao border font-bold py-2 px-4 rounded flex items-center justify-center gap-2 disabled:opacity-50 disabled:cursor-not-allowed"
                  >
                    <Clipboard size={24} />
                  ...
                  <button
                    ...
                    onClick={sendMessage}
                    className="bg-roxazul mx-auto mt-4 transition-colors hover:bg-azulao text-white font-bold py-2 px-4 rounded flex items-center justify-center gap-2 disabled:opacity-50 disabled:cursor-not-allowed"
                  >
            ...
            <div className="lg:w-1/2 w-11/12 flex flex-col items-center gap-3">
              {status === "A avaliar" ? (
                <h1 className="text-black text-center text-xl font-bold">Aguardando a avaliação do cliente...</h1>
              ) : (
                <>
                  {!(status === "Concluido") && (
                    <h1 className="text-black text-center text-xl font-bold">
                      Coloque uma imagem do serviço (Opcional):
                    </h1>
                  )}
                  <label
                    htmlFor="orcamento"
                    className="text-black text-center w-10/12 min-h-[60px] max-h-64 bg-[#EDEDED] cursor-pointer hover:bg-[#d9d9d9] transition-colors rounded-2xl flex items-center justify-center text-xl font-medium"
                  >
                  ...
                  >
                    {isSubmitting ? (
                      ...
                      <>
                        ...
                        <span className="font-bold text-xl text-white flex justify-center items-center ml-1">
                          Finalizar serviço
                        </span>
                      </>
                    )}
