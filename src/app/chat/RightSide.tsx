    ...
    function MessageItem({ content, sendHour, ...props }: Message) {
        return (
            <div className={twMerge("w-full px-2 flex items-center z-10 justify-end", userData._id === props.senderId ? "justify-end" : "justify-start")}>
                <div className={twMerge("border w-fit max-w-[55%] flex flex-col rounded-2xl p-4 text-white items-center justify-center", userData._id === props.senderId ? "bg-azulinho justify-end" : "bg-white text-black justify-start")}>
                    <h1>
                        {content}
                    </h1>
