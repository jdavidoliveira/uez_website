  ...
  return (
      <main className="w-full flex flex-col items-center mx-0">
        <ImageSection imageUrl="/images/p1.jpg">
          ...
          <button className="bg-azulao py-1 px-4 rounded-full font-bold text-white absolute bottom-6 left-1/2 -translate-x-1/2 transition hover:scale-105">Torne-se um Cliente</button>
          <div className="w-40 h-40 bg-azulao absolute bottom-0 left-0 -translate-x-1/2 rounded-r-full"></div>
        </ImageSection>
        <ImageSection imageUrl="/images/p2.jpg">
          ...
          <button className="bg-white py-1 px-4 rounded-full font-bold text-azulao absolute bottom-6 left-1/2 -translate-x-1/2 transition hover:scale-105">Torne-se um Uzer</button>
          <div className="w-20 h-40 bg-roxazul absolute bottom-0 right-0 translate-y-1/2 rounded-l-full"></div>
        </ImageSection>
        <GenericSection className="relative flex flex-col items-center justify-around gap-8 text-azulao mb-28">
          <div className="w-40 h-40 bg-roxazul absolute top-1/4 left-0 -translate-x-1/2 rounded-r-full"></div>
          <div className="w-20 h-40 bg-roxazul absolute top-3/4 right-0 rounded-l-full"></div>
          <div className="w-40 h-40 bg-roxazul absolute bottom-0 left-0 -translate-x-1/2 translate-y-1/2 rounded-r-full"></div>
          <div className="relative flex flex-col items-center justify-around gap-16 text-azulao w-11/12">
          ...
          <div className="relative flex flex-col items-center justify-around gap-8 text-azulao w-11/12">
            ...
            <div className="flex flex-col gap-4 items-center w-full">
              <div className={`flex gap-4 items-center justify-around w-full`}>
                <CardPhoto imageUrl="/images/cliente.png" title="Cliente" description="Uma pessoa que necessita de um serviço, por exemplo uma pessoa com a tubulação de casa quebrada." />
                <CardPhoto imageUrl="/images/uzer.png" title="Uzer" description="Um profissional que precisa de um serviço, por exemplo um profissional de encanamento que precisa de um cliente." />
              </div>
              <div className="flex gap-4 items-center justify-center w-full">
                <img src="/vetores/linha-baixo.svg" className="w-6/12" />
              </div>
              <div className="flex gap-4 items-center justify-center w-full">
                <div className="w-40 h-40 transition hover:scale-105" onClick={toTop} title="Uez Company">
                  <img src="/images/black_logo.svg" className="w-40 h-40 transition hover:scale-105" />
                </div>
              </div>
              <div className="flex gap-4 items-center justify-center w-full">
                <img src="/vetores/linha-cima.svg" className="w-6/12" />
              </div>
              <div className="flex gap-4 items-center justify-around w-full">
        ...
        <GenericSection className="relative flex flex-col items-center justify-around gap-8 text-azulao mb-20">
          <div className="relative flex flex-col items-center justify-around gap-8 text-azulao w-11/12">
            <img src="/images/icons/cadeado.svg" className="w-12 h-12 absolute top-8 left-0" />
          ...
          <div className="relative flex flex-col items-center justify-around gap-8 text-azulao w-11/12">
            <img src="/images/icons/cadeado.svg" className="w-12 h-12 absolute top-0 right-0" />
...
const GenericSection = ({ className = "", children }:{className: string ,children: React.ReactNode}) => (
  <section
    className={`w-full px-0 py-16 flex flex-row items-center justify-around gap-8 text-azulao animate-transitionY ${className}`}
  >{children}</section>
);

const ImageSection = ({ imageUrl, children }: { imageUrl: StaticImageData | string, children: React.ReactNode }) => (
  <section
...
const CardPhoto = ({ imageUrl, title, description }: { imageUrl: string, title: string, description: string }) => (
  <div className="w-8/12 flex flex-col items-center justify-between gap-2 transition hover:scale-105">
    <img className="w-3/12" src={imageUrl} alt="Imagem ilustrativa" />
    <h2 className="text-4xl font-normal text-center">{title}</h2>
    <p className="w-6/12 text-sm leading-tight font-medium text-center">{description}</p>
  </div>
)